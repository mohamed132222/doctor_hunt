...
class ClinicMap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final markerSize = context.w(AppSize.detailsMapMarker);
    final dotSize = context.w(AppSize.detailsMapDot);

    return SizedBox(
      height: context.w(AppSize.detailsMapHeight),
      width: double.infinity,
      child: FlutterMap(
        options: MapOptions(
          initialCameraFit: CameraFit.bounds(
            bounds: LatLngBounds(kUserLocation, kClinicLocation),
            padding: EdgeInsets.all(context.w(AppSize.s24)),
          ),
          ...
          PolylineLayer(
            polylines: [
              Polyline(
                ...
                strokeWidth: context.w(AppSize.s4),
              ),
            ],
          ),
          MarkerLayer(
